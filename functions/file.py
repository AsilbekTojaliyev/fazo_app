...
def create_file(new_files, source, source_id, db, user):
    if user.role == "admin":
        if (source == "laptop" and db.query(Laptops).filter(Laptops.id == source_id).first() is None) or \
                (source == "planshet" and db.query(Planshets).filter(Planshets.id == source_id).first() is None) or \
                (source == "telephones" and db.query(Telephones).filter(Telephones.id == source_id).first() is None):
