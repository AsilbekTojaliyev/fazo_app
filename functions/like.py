...
def create_like(source, source_id, db, user):
    if (source == "laptop" and db.query(Laptops).filter(Laptops.id == source_id).first() is None) or \
            (source == "planshet" and db.query(Planshets).filter(
                Planshets.id == source_id).first() is None) or \
            (source == "telephone" and db.query(Telephones).filter(
                Telephones.id == source_id).first() is None):
